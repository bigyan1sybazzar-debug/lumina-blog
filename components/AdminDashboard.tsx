...
export const Admin: React.FC = () => {
  ...
  // Refs for editor helpers
  const outlineRef = useRef<HTMLTextAreaElement>(null);
  const contentRef = useRef<HTMLTextAreaElement>(null);
  const excerptRef = useRef<HTMLTextAreaElement>(null);

  const handleInsertLink = (ref: React.RefObject<HTMLTextAreaElement>, setter: (val: string) => void, isInternal: boolean = false) => {
    ...
    const url = prompt(isInternal ? 'Internal Path (e.g. /my-post):' : 'External URL (e.g. https://google.com):', isInternal ? '/' : 'https://');
    if (!url) return;

    const currentVal = textarea.value;
    const linkMarkdown = `[${text}](${url})`;
    const newVal = currentVal.substring(0, start) + linkMarkdown + currentVal.substring(end);
  ...
  const handleUpdateLiveLink = async () => {
    ...
    try {
      ...
      const poll = (newLivePollTeamA && newLivePollTeamB) ? {
        teamA: newLivePollTeamA,
        teamB: newLivePollTeamB,
        votesA: editingLiveLink.poll?.votesA || 0,
        votesB: editingLiveLink.poll?.votesB || 0,
        votedUserIds: editingLiveLink.poll?.votedUserIds || []
      } : undefined;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700">
                      <div className="md:col-span-2">
                        <h4 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2 mb-2">
                          <Vote size={14} className="text-primary-500" />
                          Live Poll (Optional)
                        </h4>
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Team A Name</label>
                        <input
                          type="text"
                          value={newLivePollTeamA}
                          onChange={(e) => setNewLivePollTeamA(e.target.value)}
                          placeholder="e.g. Manchester City"
                          className="input-field"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Team B Name</label>
                        <input
                          type="text"
                          value={newLivePollTeamB}
                          onChange={(e) => setNewLivePollTeamB(e.target.value)}
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => handleInsertLink(outlineRef, setOutline, true)} className="text-xs px-3 py-1 bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/40 transition-colors flex items-center gap-1.5 font-bold"><LinkIcon size={12} /> Internal</button>
                                <button onClick={() => handleInsertLink(outlineRef, setOutline, false)} className="text-xs px-3 py-1 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors flex items-center gap-1.5 font-bold"><ExternalLink size={12} /> External</button>
                              </div>
                            </div>
                            <textarea
                              ref={outlineRef}
                            ...
                            <div>
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <div className="flex gap-1">
                                  <button onClick={() => handleInsertLink(excerptRef, setExcerpt, true)} className="text-[10px] px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center gap-1"><LinkIcon size={10} /> Internal</button>
                                  <button onClick={() => handleInsertLink(excerptRef, setExcerpt, false)} className="text-[10px] px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center gap-1"><ExternalLink size={10} /> External</button>
                                </div>
                              </div>
                              <textarea
                                ref={excerptRef}
                        ...
                        <div className="space-y-6">
                          ...
                          <div>
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => handleInsertLink(contentRef, setFullContent, true)} className="text-xs px-3 py-1.5 bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/40 transition-colors flex items-center gap-1.5 font-bold"><LinkIcon size={14} /> Add Internal Link</button>
                                <button onClick={() => handleInsertLink(contentRef, setFullContent, false)} className="text-xs px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors flex items-center gap-1.5 font-bold"><ExternalLink size={14} /> Add External Link</button>
                              </div>
                            </div>
                            <textarea
                              ref={contentRef}
